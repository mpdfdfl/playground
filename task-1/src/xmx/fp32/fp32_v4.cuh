...
{
// 8 16 block
__global__ void gemm_v4(const size_t M, const size_t N, const size_t K,
                        ...
                        float32_t* const __restrict__ C)
{
    ...
    int32_t load_a_smem_m = tid;
    int32_t load_a_smem_k = 0;
    int32_t load_b_smem_k = tid >> 4;
    int32_t load_b_smem_n = (tid & 15) << 3;
    ...
    for (int32_t bk = 0; bk < (K + BK - 1) / BK; bk++) {
        ...
        FLOAT4(s_a[load_a_smem_m][load_a_smem_k + 4]) =
            FLOAT4_CONST(A[OFFSET(load_a_gmem_m, load_a_gmem_k + 4, K)]);
        FLOAT4(s_b[load_b_smem_k][load_b_smem_n]) =
            FLOAT4_CONST(B[OFFSET(load_b_gmem_k, load_b_gmem_n, N)]);
        FLOAT4(s_b[load_b_smem_k][load_b_smem_n + 4]) =
            FLOAT4_CONST(B[OFFSET(load_b_gmem_k, load_b_gmem_n + 4, N)]);
        __syncthreads();
