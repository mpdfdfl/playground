...
{
// 32 16
__global__ void gemm_v5(const size_t M, const size_t N, const size_t K,
                        ...
                        float32_t* const __restrict__ C)
{
    ...
    int32_t load_a_smem_m = tid >> 2;
    int32_t load_a_smem_k = (tid & 3) << 1;
    int32_t load_b_smem_k = tid >> 6;
    int32_t load_b_smem_n = (tid & 63) << 1;
    ...
    for (int32_t bk = 0; bk < (K + BK - 1) / BK; bk++) {
        ...
        s_a[load_a_smem_m][load_a_smem_k] =
            A[OFFSET(load_a_gmem_m, load_a_gmem_k, K)];
        s_a[load_a_smem_m][load_a_smem_k + 1] =
            A[OFFSET(load_a_gmem_m, load_a_gmem_k + 1, K)];

        s_b[load_b_smem_k][load_b_smem_n] =
            B[OFFSET(load_b_gmem_k, load_b_gmem_n, N)];
        s_b[load_b_smem_k][load_b_smem_n + 1] =
            B[OFFSET(load_b_gmem_k, load_b_gmem_n + 1, N)];
