...
// blockDIM 32 2 4
__global__ void gemm_fp16_v19(const float16_t* A, const float16_t* B,
                              float16_t* const C, int M, int N, int K)
{
    ...
    size_t by = ((bid / 32) % 2) ? (15 - ((bid % 32) / 2)) : ((bid % 32) / 2);
...
#pragma unrool
    for (int i = 0; i < 4; i++) {
        ...
        asm volatile(
            "ldmatrix.sync.aligned.m8n8.x4.shared.b16 { %0, "
            "%1, %2, %3 }, [ "
            "%4 ];\n"
            : "=r"(FragA[frag_store][i][0]), "=r"(FragA[frag_store][i][1]),
              "=r"(FragA[frag_store][i][2]), "=r"(FragA[frag_store][i][3])
            : "r"(smem_base));
    }
    // load fragB
#pragma unrool
    for (int i = 0; i < 4; i++) {
        ...
        asm volatile("ldmatrix.sync.aligned.m8n8.x4.trans.shared.b16 "
                     ...
                     "[ %4 "
                     "];\n"
                     : "=r"(FragB[frag_store][i * 2][0]),
                       "=r"(FragB[frag_store][i * 2][1]),
                       "=r"(FragB[frag_store][i * 2 + 1][0]),
                       "=r"(FragB[frag_store][i * 2 + 1][1])
                     : "r"(smem_base));
    }
    //////////////////////////////////////////
...
#pragma unroll
            for (int nii = 0; nii < NII / mmaN; nii += 1) {
                ...
                asm volatile(
                    ...
                    : "r"(FragA[frag_load][mii][0]),
                      "r"(FragA[frag_load][mii][1]),
                      "r"(FragA[frag_load][mii][2]),
                      "r"(FragA[frag_load][mii][3]),
                      "r"(FragB[frag_load][_nii][0]),
                      "r"(FragB[frag_load][_nii][1]), "r"(Accum[mii][_nii][0]),
                      "r"(Accum[mii][_nii][1]));
            }
        }

        smem_sel_next = (smem_sel_next + 1) % 4;
