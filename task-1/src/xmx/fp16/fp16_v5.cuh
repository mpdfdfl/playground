                          ...
                          int ko)
{
    ...
    for (int i = 0; i < 4; i++) {
        ...
        int load_a_smem_addr =
            s_a_base_addr +
            OFFSET(load_a_smem_m, load_a_smem_k, 32) * sizeof(float16_t);
                          ...
                          int ko)
{
    ...
    for (int i = 0; i < 4; i++) {
        ...
        int load_b_smem_addr =
            s_b_base_addr +
            OFFSET(load_b_smem_k, load_b_smem_n, 128) * sizeof(float16_t);
...
__global__ void gemm_fp16_v5(const float16_t* A, const float16_t* B,
